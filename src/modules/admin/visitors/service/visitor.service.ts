...
export default {
  ...
  /**-------------------------------------------------- */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  updateVisitor: async (payload: any) => {
    try {
      const formData = new FormData();
    Object.entries(payload).forEach(([key, value]) => {
      if (key === 'visitorImg' && value instanceof File) {
    ...
    const response = await axios({
      method: "PUT",
      url: `${import.meta.env.VITE_SERVER_URL}/api/v1/visitor/update_visitor`,
      data: formData,
      headers: {
        'Content-Type': 'multipart/form-data',
      },
  ...
  /**-------------------------------------------------- */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  updateApprovedVisitorStatus: async (payload: any) => {
    try {
  
      const response = await axios({
      method: "PUT",
      url: `${import.meta.env.VITE_SERVER_URL}/api/v1/visitor/approved_visitor_status`,
      data: payload,
      });
